...
from dtr5app.serializers import (
...
from dtr5app.utils_search import (
...
def filter_members_view(request):
    # TODO: use DRF here.
    """
    Receive a list of Reddit usernames and return a list of
    Reddmeet users with some basic data for each.
    ...
    ul = User.objects.filter(username__in=usernames).prefetch_related("profile")[:50]
...
def user_detail(request, username, format=None):
    ...
    view_user.profile.set_viewer_latlng(
    ...
    show_created = view_user.profile.created and view_user.profile.created > date(
        1970, 1, 1
    )
    ...
    return Response(
        data={
            ...
            "common_subs": SubscribedSerializer(
                view_user.profile.get_common_subs(request.user), many=True
            ).data,
            "not_common_subs": SubscribedSerializer(
                view_user.profile.get_not_common_subs(request.user), many=True
            ).data,
...
def upvotes_recv_api(request, format=None):
    ...
    nopes_qs = User.objects.filter(
        flags_received__sender=request.user, flags_received__flag=Flag.NOPE_FLAG
    )
    ...
    return JsonResponse(
        data={"user_list": BasicUserSerializer(user_list, many=True).data,}
    )
...
def matches_api(request, format=None):
    ...
    request.user.profile.save(update_fields=["matches_count", "new_matches_count"])
    return JsonResponse(
        data={"user_list": BasicUserSerializer(user_list, many=True).data,}
    )
...
def upvotes_sent_api(request, format=None):
    ...
    user_list = (
        User.objects.filter(
            flags_received__sender=request.user, flags_received__flag=Flag.LIKE_FLAG
        )
        .annotate(flag_created=F("flags_received__created"))
        .prefetch_related("profile")
        .order_by("-flags_received__created")[:500]
    )
...
def visitors_api(request, format=None):
    ...
    vl = (
        request.user.was_visited.filter(
            visitor__last_login__isnull=False, visitor__is_active=True, hidden=False
        )
        .order_by("-visitor")
        .distinct("visitor")
    ...
    user_list.object_list = add_matches_to_user_list(
        user_list.object_list, request.user
    )
...
def update_search_if_changed(opts, user, session_obj=None):
    ...
    if settings.DEBUG:
        print(
            "# search_results_order == {}".format(session_obj["search_results_order"])
        )
